...
<div class="main-content">
    ...
    <div class="content-area">
        ...
        <?php else: ?>
            <div class="card">
                <div class="card-body">
                    <form method="post" action="<?php echo BASE_URL; ?>/inventory_issue/update/<?php echo $item['id']; ?>">
                        <div class="row">
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <label>Loại</label>
                                    <select name="issue_type" class="form-select">
                                        <option value="sale" <?php echo ($item['issue_type'] == 'sale') ? 'selected' : ''; ?>>sale</option>
                                        <option value="manual" <?php echo ($item['issue_type'] == 'manual') ? 'selected' : ''; ?>>manual</option>
                                        <option value="waste" <?php echo ($item['issue_type'] == 'waste') ? 'selected' : ''; ?>>waste</option>
                                    </select>
                                </div>

                                <div class="mb-3">
                                    <label>Ngày</label>
                                    <input type="date" name="issue_date" class="form-control" value="<?php echo htmlspecialchars($item['issue_date']); ?>" required>
                                </div>
...
<script>
    (function() {
        ...
        if (Array.isArray(details)) {
            details.forEach(d => addRow(d.ingredient_id, d.qty));
        } else {
            addRow('', 1);
        }
